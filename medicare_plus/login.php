...
if($_SERVER['REQUEST_METHOD']=='POST'){
    ...
    if($user && password_verify($password,$user['password_hash'])){
        ...
        else header('Location: dashboard_patient.php');
    } else {
...
<head>
<meta charset="UTF-8">
<title>Login - MediCare Plus | 2026</title>
...
<header class="text-center py-12 relative z-10">
  ...
  <p class="mt-2 text-sky-300 tracking-widest text-sm">User Login • 2026</p>
...
<section class="flex-1 flex items-center justify-center px-6 mt-12 reveal">
  <div class="glass-card p-10 rounded-3xl shadow-glow max-w-md w-full">
    <h2 class="text-3xl md:text-4xl font-bold neon-text mb-6 text-center">User Login</h2>
    ...
    <form method="POST" class="flex flex-col gap-4">
      ...
      <button type="submit" class="f-btn mt-2">Login</button>
    </form>

    <p class="mt-4 text-center text-sky-300">
      Don't have an account? <a href="register.php" class="underline hover:text-white">Register</a>
    </p>
  </div>
</section>
